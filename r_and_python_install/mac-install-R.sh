...
if ! command -v r &>/dev/null || [ "$1" = "-f" ]; then
  ...
  if grep -q '/Library/Frameworks/R.framework/Resources'; then
    echo -e '# R Framework.\n[ -d "/Library/Frameworks/R.framework/Resources" ] && export PATH="/Library/Frameworks/R.framework/Resources:$PATH"' >>~/.bash_profile
  fi
