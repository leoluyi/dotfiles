...
All-in-one LLM CLI with shell assistant, REPL, roles, sessions, RAG, and function calling ([aichat](https://github.com/sigoden/aichat)).

**Requirements:** `aichat` installed (`brew install aichat`) and an API key set via environment variable (e.g. `OPENAI_API_KEY`).

Config is stowed from `common_dotfiles/.config/aichat/config.yaml`. The `models-override.yaml` is auto-generated by `aichat --sync-models` and is not tracked.

Default model: `openai:gpt-5.2` (set via `model` in `config.yaml`).

### Shell Integration (`~/.config/bash/43_aichat`)

| Alias | Usage | Description |
|---|---|---|
| `??` | `?? <question>` or `<cmd> \| ??` | General-purpose AI query (alias for `aichat`) |

### Key CLI Usage

| Command | Description |
|---|---|
| `aichat "question"` | One-shot query |
| `aichat -e "task description"` | Shell assistant — generates a command and prompts to execute |
| `aichat -c "task"` | Code-only output |
| `aichat -r <role> "prompt"` | Query using a named role |
| `aichat -s` | Start an interactive session with context |
| `<cmd> \| aichat` | Pipe output as input |
| `aichat --serve` | Start a local OpenAI-compatible API server |

### Roles

Custom roles live in `~/.config/aichat/roles/<name>.yaml`. Each role defines a system prompt and optional model settings.
