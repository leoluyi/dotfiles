...
map("n", "<leader>ai", function()
  ...
  require("util.llm").query_replace_auto(vim.api.nvim_get_current_buf(), line, line)
end, { desc = "AI: improve text (replace line)" })

map("x", "<leader>ai", function()
  local start_line = vim.fn.line("'<")
  local end_line = vim.fn.line("'>")
  require("util.llm").query_replace_auto(vim.api.nvim_get_current_buf(), start_line, end_line)
end, { desc = "AI: improve text (replace selection)" })

map("n", "<leader>ab", function()
  local line = vim.fn.line(".")
  require("util.llm").query_replace(vim.api.nvim_get_current_buf(), line, line, { role = "blog" })
end, { desc = "AI: blog style (replace line)" })

map("x", "<leader>ab", function()
  local start_line = vim.fn.line("'<")
  local end_line = vim.fn.line("'>")
  require("util.llm").query_replace(vim.api.nvim_get_current_buf(), start_line, end_line, { role = "blog" })
end, { desc = "AI: blog style (replace selection)" })

map("n", "<leader>ac", function()
  local line = vim.fn.line(".")
  require("util.llm").query_replace(vim.api.nvim_get_current_buf(), line, line, { cmd = { "aichat", "-S", "-r", "%code%" } })
end, { desc = "AI: code only (replace line)" })

map("x", "<leader>ac", function()
  local start_line = vim.fn.line("'<")
  local end_line = vim.fn.line("'>")
  require("util.llm").query_replace(vim.api.nvim_get_current_buf(), start_line, end_line, { cmd = { "aichat", "-S", "-r", "%code%" } })
