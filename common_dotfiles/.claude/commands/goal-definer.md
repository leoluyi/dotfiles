---
description: Turn a fuzzy task into a six-element goal prompt an agent can run for hours without drifting or stopping early
argument-hint: [rough task description, or blank to be asked]
effort: high
---
...
<scope>
If $ARGUMENTS is non-empty, treat it as the user's rough task description and skip the opening question in step 1 — go straight to step 2 (Outcome). If it is empty, start at step 1.
</scope>
...
3. Verification — how the agent proves completion without you eyeballing it:
 ...
 - If the user has a Taste Profile from Taste Distiller (`/taste-distiller`), ask them to paste it here as the verification standard.
 - Wait for their answer.
...
- This command is an interactive multi-turn interview. Do not delegate it to a subagent, and do not batch all questions into one message.
</guardrails>
