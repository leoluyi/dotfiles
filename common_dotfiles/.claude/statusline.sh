...
# Mirrors ECC suggest-compact's context signal (scripts/lib/transcript-context.js):
# it suggests /compact once context tokens cross a window-scaled threshold, so the
# statusline shows the same number the hook warns against — not the raw window %.
#   threshold = COMPACT_CONTEXT_THRESHOLD  (>0 wins, =0 disables the indicator)
#            else  window >= 1M ? 250k : 160k
#   window    = ECC_CONTEXT_WINDOW_TOKENS / CLAUDE_CODE_AUTO_COMPACT_WINDOW override,
#               else the window size Claude Code reports on stdin.
compact_window="$size"
if [ "${ECC_CONTEXT_WINDOW_TOKENS:-0}" -gt 0 ] 2>/dev/null; then
    compact_window="$ECC_CONTEXT_WINDOW_TOKENS"
elif [ "${CLAUDE_CODE_AUTO_COMPACT_WINDOW:-0}" -gt 0 ] 2>/dev/null; then
    compact_window="$CLAUDE_CODE_AUTO_COMPACT_WINDOW"
fi

compact_threshold=-1
if [ -n "${COMPACT_CONTEXT_THRESHOLD:-}" ] && [ "$COMPACT_CONTEXT_THRESHOLD" -ge 0 ] 2>/dev/null; then
    compact_threshold="$COMPACT_CONTEXT_THRESHOLD"
fi
if [ "$compact_threshold" -lt 0 ] 2>/dev/null; then
    if [ "$compact_window" -ge 1000000 ] 2>/dev/null; then
        compact_threshold=250000
    else
...
if [ "$compact_threshold" -gt 0 ] 2>/dev/null; then
    thr_pct=$(( current * 100 / compact_threshold ))
fi
...
if [ "$thr_pct" -ge 0 ] 2>/dev/null; then
    thr_color=$(color_for_pct "$thr_pct")
