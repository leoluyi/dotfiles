...
if command -v pyenv &>/dev/null && command -v pyenv-virtualenv &>/dev/null; then
  ...
  pyenv activate py3nvim && pip install --no-cache-dir -U neovim pynvim jedi black flake8 autopep8 isort
  echo -e "Note the path and add it to your 'init.vim' as below: \nlet g:python3_host_prog = '$(pyenv which python)'"
